...
│   └── publicController.ts        # 반경 내 공공시설 조회 (GET /api/public/nearby)
├── routes/
...
│   └── public.ts             # GET /api/public/nearby
├── config/
...
├── __tests__/
│   └── schedule.test.ts      # Schedule API 테스트 (25개 케이스, Prisma 모킹)
└── jobs/
...
├── environments/
│   ├── local.bru             # 로컬 환경 변수 (localhost:3000)
│   └── production.bru        # 프로덕션 환경 변수 (tour-cast.fly.dev)
└── schedule/
    ├── 01-create-travel-plan.bru
    ├── 02-get-map-items.bru
    ├── 03-get-route.bru
    ├── 04-get-list.bru
    ├── 05-update-item-status.bru
    ├── 06-get-heatmap.bru
    └── error-cases/          # 400/404 에러 케이스
```
...
| `Schedule` | `schedules` | 단건 일정 (FCM 알림용 레거시) |
| `TravelPlan` | `travel_plans` | 여행 플랜 그룹 (Plango 앱 연동) |
| `TravelPlanItem` | `travel_plan_items` | 여행 플랜 개별 일정 아이템 |

공공데이터 모델은 `contentId`를 unique key로 사용합니다. `Schedule`은 `deviceToken`·`notificationSent` 필드로 FCM 알림 상태를 관리합니다. `TravelPlan`은 `userId`·`date` 기반으로 복수의 `TravelPlanItem`을 가지며, 아이템 삭제 시 Cascade가 적용됩니다.
...
| `category` | `string` | `restaurant` \| `attraction` \| `accommodation` \| `transport` \| `other` |
| `time` | `string` | `HH:mm` 형식 표시 시간 |
| `scheduledAt` | `DateTime?` | ISO 8601 정확한 일시 (선택) |
| `order` | `int` | 당일 내 순서 |
...
| `publicFacilityService` | `apis.data.go.kr/B551182/MedGeoInfo` | 반경 내 병·의원·약국 조회 (HIRA) |
...
[Bruno](https://www.usebruno.com) 앱에서 `bruno/` 폴더를 열어 API를 테스트할 수 있습니다.

1. Bruno 앱 실행 → **Open Collection** → 프로젝트의 `bruno/` 폴더 선택
2. 우상단 환경 드롭다운에서 **local** 선택
3. `schedule/01-create-travel-plan` 먼저 실행 → `planId`·`itemId`가 환경 변수에 자동 저장
4. 이후 요청들이 저장된 변수를 자동으로 사용

| 환경 | baseUrl |
|------|---------|
| `local` | `http://localhost:3000` |
| `production` | `https://tour-cast.fly.dev` |
...
| Method | Path | 설명 |
|--------|------|------|
| POST | `/api/schedule` | 여행 플랜 저장 (다중 아이템) |
| GET | `/api/schedule/map` | 지도용 일정 아이템 조회 |
| GET | `/api/schedule/route` | 날짜별 경로·거리·소요시간 조회 |
| GET | `/api/schedule/heatmap` | 방문 히트맵 조회 |
| GET | `/api/schedule/list` | 저장된 여행 플랜 목록 조회 |
| PATCH | `/api/schedule/item/:itemId` | 아이템 상태 업데이트 |
| POST | `/api/schedule/single` | 단건 일정 생성 (FCM 알림용 레거시) |
| GET | `/api/schedule?userId=` | 단건 일정 목록 조회 (레거시) |
| GET | `/api/schedule/:id` | 단건 일정 단건 조회 (레거시) |
| PUT | `/api/schedule/:id` | 단건 일정 수정 (레거시) |
| DELETE | `/api/schedule/:id` | 단건 일정 삭제 (레거시) |

#### POST /api/schedule — 여행 플랜 저장
...
```json
{
  ...
  "title": "서울 당일 여행",
  "items": [
    { "id": "item_cuid", "title": "경복궁", "status": "pending", ... }
  ]
}
```

#### GET /api/schedule/map — 지도용 아이템 조회

쿼리 파라미터: `userId` (필수), `date` YYYY-MM-DD (필수), `filters` 카테고리 쉼표 구분 (선택)
...
> 반드시 `{ items: [...] }` 래핑 형식으로 반환합니다. 배열 직접 반환 시 앱에서 오류 발생.

#### GET /api/schedule/route — 경로 조회

쿼리 파라미터: `userId` (필수), `date` YYYY-MM-DD (필수)
...
- `totalDistance`: km (소수점 1자리)
- `estimatedTime`: 분 (정수, 평균 50km/h 기준 Haversine 계산)
...
```json
[
  { "lat": 37.5796, "lng": 126.9770, "weight": 3 },
  { "lat": 37.5512, "lng": 126.9882, "weight": 1 }
]
```

- `weight`: 해당 좌표(소수점 4자리 기준) 누적 방문 횟수

#### GET /api/schedule/list — 플랜 목록 조회

쿼리 파라미터: `userId` (필수), `page` (기본값 1), `limit` (기본값 20)
...
응답 (200): `{ "id": "item_cuid", "status": "completed" }`

허용 값: `pending` | `in_progress` | `completed` | `cancelled`

### 공공시설

| Method | Path | 설명 |
|--------|------|------|
| GET | `/api/public/nearby` | 반경 내 병·의원·약국 거리순 조회 |

#### GET /api/public/nearby 쿼리 파라미터

| 파라미터 | 필수 | 기본값 | 설명 |
|---------|------|-------|------|
| `lat` | ✓ | — | 기준 위도 |
| `lng` | ✓ | — | 기준 경도 |
| `radius` | | 1000 | 검색 반경 m (최대 5000) |
| `type` | | hospital | `hospital` / `pharmacy` / `all` |

#### GET /api/public/nearby 응답 예시

```bash
GET /api/public/nearby?lat=37.5662&lng=126.9779&radius=500&type=all
```
...
- HIRA `getMedBasisList` API를 `DIST`(km 정수) 단위로 호출 후 Haversine으로 정밀 필터링
- `type=all` 시 병·의원과 약국을 `Promise.allSettled`로 **병렬 조회**
- `PUBLIC_DATA_API_KEY` 미설정 시 503 반환

#### 일정 생성 시 자동 조회

`POST /api/schedule`에서 `lat`/`lng` 미제공으로 자동 지오코딩이 실행된 경우, 해당 좌표 **반경 500m** 내 공공시설(병·의원+약국)을 **병렬·fail-open**으로 조회하여 응답에 포함합니다.
...
```json
{
  ...
  "nearbyFacilities": [
    { "name": "종로구보건소", "distance": 380, "type": "보건소" }
  ]
}
```
...
```bash
# 개발 환경 (Docker DB 기동 후)
docker compose up -d db redis
npx prisma migrate dev --name add-travel-plan
...
| 테스트 파일 | 케이스 수 | 대상 |
|------------|---------|------|
| `src/__tests__/schedule.test.ts` | 25 | TravelPlan 생성·조회·상태 업데이트 + 단건 CRUD 정상/오류 케이스 |

## 유틸리티

### Haversine 거리 계산 (`src/utils/haversine.ts`)

WGS84 위경도 두 점 사이의 구면 거리를 계산합니다. 공공시설 반경 필터링에 사용됩니다.

```typescript
import { haversineM, haversineKm } from './utils/haversine';

haversineM(37.5662, 126.9779, 37.5010, 127.0051);  // → 8237 (m)
haversineKm(37.5662, 126.9779, 37.5010, 127.0051); // → 8.237 (km)
```
...
1. [공공데이터포털](https://www.data.go.kr) 접속 → 로그인
2. **건강보험심사평가원 요양기관 위치정보 서비스** 검색 → **활용 신청**
3. 승인 후 **마이페이지 → 개발계정**에서 **일반 인증키(Encoding)** 복사
4. `.env`에 입력

```bash
PUBLIC_DATA_API_KEY=your_encoding_key_here
```

> 활용 신청 후 승인까지 수 분~1일 소요될 수 있습니다. 같은 키로 관광공사 API(`TOUR_API_KEY`)와 별개 신청이 필요합니다.
